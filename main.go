...
func enableQuickEditMode() {
	...
	// 正确方式：将 syscall.STD_INPUT_HANDLE 转为 int32 再转 uintptr
	handle := int32(syscall.STD_INPUT_HANDLE) // -10
	stdin, _, _ := getStdHandle.Call(uintptr(handle))
	if stdin == 0 {
		return
	}
